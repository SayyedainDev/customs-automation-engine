...
Answers are **generated by Groq from already-gated evidence**, at the project
owner's explicit request, and the safety property is enforced around that call
rather than by avoiding it:

* retrieval, the evidence gate and the deterministic checklist all run first,
  so the model only ever sees passages the gate already accepted;
* the model writes prose only. Which documents are required, and every
  passed/failed/manual_review value, is decided by Python before the call and
  is rendered verbatim beneath the generated text;
* generated prose passes the same forbidden-claim validation as CACE's own
  framing, plus a check that it names no document outside the deterministic
  checklist and makes no completeness claim - it is discarded for the written
  template if it fails;
* a retrieved passage is data in the prompt, never an instruction: the system
  prompt says so explicitly, and nothing in this path executes passage text.

An earlier version of this module composed answers extractively and called no
model at all. That is no longer true, and this docstring said otherwise.
"""
