...
class GroqNarrator:
    ...
    def __call__(self, role: str, findings: dict[str, Any]) -> str:
        ...
        if is_explanation:
            role_instruction = (
                "Write a plain-language explanation of 150-260 words for a "
                "reader with no background in customs, programming or AI. "
                "Use short paragraphs, active voice and sentences under 20 "
                "words when practical. Use these six headings, in this "
                "order, each on its own line: 'Decision', 'What the system "
                "checked', 'Why this decision', 'Regulatory evidence', "
                "'What to do next', 'Limitations'. Never omit a heading. "
                "State the result in Decision before discussing anything "
                "else. In 'Why this decision', use the exact extracted "
                "values supplied (quantities, prices, weights, PCT codes) "
                "instead of only naming the check - e.g. write 'the invoice "
                "says 100 pieces while the packing list says 99' rather than "
                "'quantity check failed'. In 'Regulatory evidence', say what "
                "was searched for and what was found in plain words; never "
                "mention BM25, embeddings, Reciprocal Rank Fusion, "
                "cross-encoder reranking, or any raw retrieval or rerank "
                "score in this explanation - only cite sources that appear "
                "in the supplied JSON. In 'What to do next', give numbered, "
                "specific actions. Never write a raw internal identifier "
                "(anything with underscores, such as item_quantity_match or "
                "xr_coo_china) - always use the plain label supplied for it. "
                "Avoid unexplained technical words such as deterministic, "
                "provenance, schema, canonical, checkpoint, interrupt, or "
                "consensus; if one is genuinely necessary, explain it in the "
                "same sentence using the word 'meaning', e.g. 'a "
                "deterministic check, meaning a fixed rule that gives the "
                "same result every time'. Never write only 'all checks "
                "passed' without naming the actual values. Never invent a "
                "finding, value or citation that is not in the supplied "
                "JSON. Treat an invoice or packing-list line reference only "
                "as a location, never as proof that the line itself is "
                "missing. Clearly distinguish initial audit inputs from "
                "supporting documents that the audit says must be obtained. "
                "Write prose sentences without markdown symbols such as * or "
                "#. This is a pre-submission document audit only, not a "
                "customs authority: never say the shipment is cleared for "
                "customs, cleared for entry, approved by customs, officially "
                "or guaranteed compliant, or that no further documents can "
                "ever be required. Never claim external authentication of a "
                "document. The 'Limitations' heading must always state that "
                "this is not official customs clearance, external document "
